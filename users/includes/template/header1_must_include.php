...
if ($settings->force_ssl == 1) {
    if (!isset($_SERVER['HTTP_HOST'])) {
        exit('HTTP_HOST must be set due to force https rule');
    }
...
if ($user->isLoggedIn() && $currentPage != 'user_settings.php' && $user->data()->force_pr == 1) {
    Redirect::to($us_url_root.'users/user_settings.php?err=You+must+change+your+password!');
}

$page = currentFile();
$titleQ = $db->query('SELECT title FROM pages WHERE page = ?', [$page]);
if ($titleQ->count() > 0) {
