...
if ($diag) {
    logger($user->data()->id, 'DIAG', 'Attempting to create zip file');
}
...
if ($diag) {
    logger($user->data()->id, 'DIAG', 'Attempting CURL request to download file contents');
}
$ch_start = curl_init();
...
if (!$page) {
    if ($diag) {
        logger($user->data()->id, 'DIAG', 'CURL Error :- '.curl_error($ch_start));
    }
...
if ($zip->open($zipFile) != 'true') {
    if ($diag) {
        logger($user->data()->id, 'DIAG', 'Error :- Unable to open the Zip File');
    }
...
if ($newCrc == $hash && $newCrc == $result) { //Note that we are checking the hash against the api call and the one supplied by ajax
    ...
    if ($diag) {
        logger($user->data()->id, 'DIAG', "The security hash DOES NOT MATCH newCRC $newCrc hash $hash result $result");
    }
    $msg = [];
    $msg['success'] = false;
