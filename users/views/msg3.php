...
<div class="modal-content">
  ...
  <div class="modal-body" id="archivediv">
    <?php $messagesQ2 = $db->query('SELECT * FROM message_threads WHERE (msg_to = ? AND archive_to = ? AND hidden_to = ?) OR (msg_from = ? AND archive_from = ? AND hidden_from = ?) ORDER BY last_update DESC', [$user->data()->id, 1, 0, $user->data()->id, 1, 0]);
            ...
            <tr>
              <?php if ($messagesCount2 > 0) {?>
                <?php foreach ($messages2 as $m2) { ?>
                  ...
                  if ($findUser->count() == 1) {
                      $foundUser = $findUser->first()->email;
                  }
                  if ($findUser->count() == 0) {
                      $foundUser = 'null@null.com';
                  }
                  ...
                  if ($difference >= 0 && $difference < 7) {
                      ...
                      $replies = $db->query('SELECT COUNT(*) AS count FROM messages WHERE msg_thread = ? GROUP BY msg_thread', [$m2->id]);
                      ...
                      <td class="pull-left">
                        <h4>
                          ...
                          <a class="nounderline" href="message.php?id=<?=$m2->id?>">
                            <?=$m2->msg_subject?> - with <?php if ($m2->msg_from == $user->data()->id) {
                          echouser($m2->msg_to);
                      } else {
                          echouser($m2->msg_from);
                      } ?>
                          </a>
                        </h4>
                        <a class="nounderline" href="message.php?id=<?=$m2->id?>">
