    ...
    if (!empty($_POST['permChange'])) {
        ...
        if ($permChange == 'Remove') {
            if (!in_array($user_id, $master_account)) {
                $checkQ = $db->query('SELECT id FROM user_permission_matches WHERE user_id = ? AND permission_id = ?', [$user_id, $permissionId]);
                $checkC = $checkQ->count();
                if ($checkC > 0) {
                    $check = $checkQ->results();
                    foreach ($check as $c) {
                        $db->query('DELETE FROM user_permission_matches WHERE id = ?', [$c->id]);
                    }
                }
            }
        }
    ...
    if (!empty($_POST['delete'])) {
        if (!in_array($permissionId, $permission_exempt)) {
            ...
            if ($deletion_count = deletePermission($deletions)) {
                $successes[] = lang('PERMISSION_DELETIONS_SUCCESSFUL', [$deletion_count]);
    ...
    if (!empty($_POST['name'])) {
        ...
        if (!empty($_POST['removePermission'])) {
            ...
            if ($deletion_count = removePermission($permissionId, $remove)) {
                $successes[] = lang('PERMISSION_REMOVE_USERS', [$deletion_count]);
        ...
        if (!empty($_POST['addPermission'])) {
            ...
            if ($addition_count = addPermission($permissionId, $add)) {
                $successes[] = lang('PERMISSION_ADD_USERS', [$addition_count]);
        ...
        if (!empty($_POST['removePage'])) {
            ...
            if ($deletion_count = removePage($remove, $permissionId)) {
                $successes[] = lang('PERMISSION_REMOVE_PAGES', [$deletion_count]);
        ...
        if (!empty($_POST['addPage'])) {
            ...
            if ($addition_count = addPage($add, $permissionId)) {
                $successes[] = lang('PERMISSION_ADD_PAGES', [$addition_count]);
                      ...
                      <td>
                        ...
                        if (!in_array($r->id, $master_account)) {
                          ...
                          <form class="" action="admin.php?view=permission&id=<?=$permissionId?>&query=<?=$query?>" method="post">
                            ...
                            <input type="hidden" name="csrf" value="<?=$token?>" >
                            <?php
                            $count = $db->query('SELECT id FROM user_permission_matches WHERE user_id = ? AND permission_id = ?', [$r->id, $permissionId])->count();
          ...
          <div class="tab-pane fade" id="AddPagesTab" role="tabpanel" aria-labelledby="AddPages">
            ...
            foreach ($pageData as $v1) {
                if ($settings->page_permission_restriction == 1) {
                    $countQ = $db->query('SELECT id, permission_id FROM permission_page_matches WHERE page_id = ? ', [$v1->id]);
