...
if (!empty($_POST)) {
    ...
    if ($action == 'archive' && isset($_POST['checkbox'])) {
        ...
        if ($deletion_count = adminArchiveThread($deletions, 'both', $user->data()->id)) {
            $successes[] = lang('MSG_ARCHIVE_SUCCESSFUL', [$deletion_count]);
        } else {
    ...
    if ($action == 'unarchive' && isset($_POST['checkbox'])) {
        ...
        if ($deletion_count = adminUnarchiveThread($deletions, 'both', $user->data()->id)) {
            $successes[] = lang('MSG_UNARCHIVE_SUCCESSFUL', [$deletion_count]);
        } else {
            $errors[] = lang('SQL_ERROR');
        }
    }
    ...
    if ($action == 'delete' && isset($_POST['checkbox'])) {
        ...
        if ($deletion_count = adminDeleteThread($deletions, 'both', $user->data()->id)) {
            $successes[] = lang('MSG_DELETE_SUCCESSFUL', [$deletion_count]);
        } else {
            $errors[] = lang('SQL_ERROR');
        }
    }
    ...
    if (!empty($_POST['messageSettings'])) {
        if ($settings->msg_notification != $_POST['msg_notification']) {
            ...
            $fields = ['msg_notification'=>$msg_notification];
        ...
        if ($settings->msg_blocked_users != $_POST['msg_blocked_users']) {
            ...
            if (empty($msg_blocked_users)) {
                $msg_blocked_users == 0;
            }
            $fields = ['msg_blocked_users'=>$msg_blocked_users];
            $db->update('settings', 1, $fields);
        ...
        if ($settings->msg_default_to != $_POST['msg_default_to']) {
            ...
            if (empty($msg_default_to)) {
                $msg_default_to == 0;
            }
            $fields = ['msg_default_to'=>$msg_default_to];
            $db->update('settings', 1, $fields);
    ...
    if (!empty($_POST['send_mass_message'])) {
        ...
        foreach ($userData as $v1) {
            ...
            if (isset($_POST['sendEmail'])) {
                $email = $db->query('SELECT fname,email,msg_notification FROM users WHERE id = ?', [$v1->id])->first();
      ...
      <div class="content mt-3">
        ...
        <?php if ($count > 0) {?><label><input type="checkbox" class="checkAllMsg" />
          ...
          <form autocomplete="off" name="threads" action="admin.php?view=messages" method="post">
          ...
          if ($difference >= 0 && $difference < 7) {
              ...
              if ($today == $last_message) {
                  $last_update = 'Today, ';
                  $last_update .= date('g:i A', $lastmessage);
              } else {
                  $last_update = date('l g:i A', $lastmessage);
              }
          } elseif ($difference >= 7) {
              $last_update = date('M j, Y g:i A', $lastmessage);
          }
          $replies = $db->query('SELECT COUNT(*) AS count FROM messages WHERE msg_thread = ? GROUP BY msg_thread', [$m->id]);
          $repliescount = $replies->count(); ?>
                          ...
                          <td class="pull-left">
                            <h4>
                              ...
                              <?php $unread = $db->query('SELECT * FROM messages WHERE msg_thread = ? AND msg_to = ? AND msg_read = ?', [$m->id, $user->data()->id, 0]);
                      ...
                      <div class="modal-dialog">
                        ...
                        <div class="modal-content">
                          ...
                          <div class="modal-body">
                            <form autocomplete="off" class="form" id="messageSettings" name='messageSettings' action='admin.php?view=messages' method='post'>
                              <div class="form-group">
                                ...
                                <select id="msg_notification" class="form-control" name="msg_notification">
                                  <option value="1" <?php if ($settings->msg_notification == 1) {
          echo 'selected="selected"';
      } ?> >Enabled</option>
                                  <option value="0" <?php if ($settings->msg_notification == 0) {
          echo 'selected="selected"';
      } ?> >Disabled</option>
                                </select>
                              </div>
                              ...
                              <div class="form-group">
                                ...
                                <select id="msg_blocked_users" class="form-control" name="msg_blocked_users">
                                  <option value="1" <?php if ($settings->msg_blocked_users == 1) {
          echo 'selected="selected"';
      } ?> >Enabled</option>
                                  <option value="0" <?php if ($settings->msg_blocked_users == 0) {
          echo 'selected="selected"';
      } ?> >Disabled</option>
                                </select>
                              </div>
                              ...
                              <div class="form-group">
                                ...
                                <select id="msg_default_to" class="form-control" name="msg_default_to">
                                  <option value="1" <?php if ($settings->msg_default_to == 1) {
          echo 'selected="selected"';
      } ?> >Yes</option>
                                  <option value="0" <?php if ($settings->msg_default_to == 0) {
          echo 'selected="selected"';
      } ?> >No</option>
                                </select>
                              </div>
                      ...
                      <div id="composemass" class="modal fade" role="dialog">
                        <div class="modal-dialog">
                          ...
                          <div class="modal-content">
                            ...
                            <div class="modal-body">
                              <form autocomplete="off" name="create_mass_message" action="admin.php?view=messages" method="post">
