...
if (Input::exists('get')) {
    ...
    if ($validation->passed()) { //if email is valid, do this
        ...
        if ($verify->data()->email_verified == 1 && $verify->data()->vericode == $vericode && $verify->data()->email_new == '') {
            ...
            $verify->update(['email_verified' => 0, 'vericode' => randomstring(15), 'vericode_expiry' => $vericode_expiry], $verify->data()->id);
            ...
            if ($verify->exists() && $verify->data()->vericode == $vericode && (strtotime($verify->data()->vericode_expiry) - strtotime(date('Y-m-d H:i:s')) > 0)) {
                ...
                if ($new == 1 && !$verify->data()->email_new == null) {
                    $verify->update(['email_verified' => 1, 'vericode' => randomstring(15), 'vericode_expiry' => date('Y-m-d H:i:s'), 'email' => $verify->data()->email_new, 'email_new' => null], $verify->data()->id);
                } else {
                    $verify->update(['email_verified' => 1, 'vericode' => randomstring(15), 'vericode_expiry' => date('Y-m-d H:i:s')], $verify->data()->id);
                }
                $verify_success = true;
                ...
                includeHook($eventhooks, 'body');
                if ($new == 1) {
                    Redirect::to($us_url_root.'users/user_settings.php?msg=Email Updated Successfully');
                }
            }
        }
    } else {
