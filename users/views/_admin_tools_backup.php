...
if (!empty($_POST)) {
    ...
    if (!empty($_POST['delete'])) {
        ...
        if ($count == 1) {
            $successes[] = "$count Backup Deleted";
        }
        if ($count > 1) {
            $successes[] = "$count Backups Deleted";
        }
    }
}
$backup_dest = '@'.$settings->backup_dest; //::from 4.2.9a
        ...
        <div class="form-group">
          ...
          <label for="backup_source" class="control-label col-sm-12">
            ...
            <select id="backup_source" class="form-control" name="backup_source">
              ...
              <option value="db_us_files" <?php if ($settings->backup_source == 'db_us_files') {
    echo 'selected="selected"';
} ?>><?=lang('AB_DB_TM_FILES'); ?></option>

              <option value="db_only" <?php if ($settings->backup_source == 'db_only') {
    echo 'selected="selected"';
} ?>><?=lang('AB_DB_FILES'); ?></option>

              <option value="us_files" <?php if ($settings->backup_source == 'us_files') {
    echo 'selected="selected"';
} ?>><?=lang('AB_TM_FILES'); ?></option>

              <option value="db_table" <?php if ($settings->backup_source == 'db_table') {
    echo 'selected="selected"';
} ?>><?=lang('AB_SINGLE_TBL'); ?></option>
            ...
            <?=lang('AB_SELECT_TBL'); ?>
              ...
              <select id="backup_table" class="form-control" name="backup_table">
                ...
                <?php foreach ($tablev as $v) { ?>
                  <option value="<?=end($v); ?>" <?php if ($settings->backup_table == end($v)) {
    echo 'selected="selected"';
} ?>><?=end($v); ?></option>
                <?php } ?>
